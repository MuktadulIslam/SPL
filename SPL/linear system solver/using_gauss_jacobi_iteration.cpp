...
void gauss_jacobi_iteration_method(char *equationFileName) {
    ...
    double newValue[row], oldValue[row], sum, Max, difference;
    for(i=0 ; i<row ; i++)
        oldValue[i] = (double)*(dMat+i) / (double)*(coefficientMatrix + i*column + i);
    ...
    for(i=0 ; i<row ; i++) {
        ...
        if((double)abs(*(coefficientMatrix + i*column + i)) < sum) {
            cout << "Coefficient Matrix is not Diagonally dominant.... Gauss Jacobi Iteration Method can't be applied on this linear system." << endl;
            return;
        }
    ...
    while(true) {
        ...
        for(i=0 ; i<row ; i++) {
            ...
            for(j=0 ; j<row ; j++) {
                if(j == i) continue;
                sum += (double)*(coefficientMatrix + i*column + j) * oldValue[j];
            }
            newValue[i] = ((double)dMat[i] - sum) / (double)*(coefficientMatrix + i*column + i);

            difference = fabs(newValue[i] - oldValue[i]);
            if(Max < difference)
                Max = difference;
        }
